...
# 设置入口节点
workflow.set_entry_point("node_entry")  # 已经和START节点绑定，外部调用时直接从node_entry开始执行

# 定义条件边的路由函数(根据是否是PDF或MD文件决定流程走向)
def route_after_entry(state: ImportGraphState) -> str:
    """
    根据入口节点的状态决定下一步的节点：
    - 如果是PDF文件，路由到 node_pdf_to_md 节点。
    - 如果是Markdown文件，路由到 node_md_img 节点。
    - 如果都不是, 直接转到END节点，结束流程。
    :param state: is_pdf_read_enabled 和 is_md_read_enabled 
...
# 定义静态边
workflow.add_edge("node_pdf_to_md", "node_md_img")  # PDF转MD后，继续处理MD图片
workflow.add_edge("node_md_img", "node_document_split")  # MD图片处理后，继续进行文档分块
workflow.add_edge("node_document_split", "node_item_name_recognition")  # 文档分块后，进行项目名识别
workflow.add_edge("node_item_name_recognition", "node_bge_embedding")  # 项目名识别后，进行BGE向量化
workflow.add_edge("node_bge_embedding", "node_import_milvus")  # BGE向量化后，导入Milvus
workflow.add_edge("node_import_milvus", END)  # 导入Milvus后，结束
...
if __name__ == "__main__":
    ...
    if not os.path.exists(test_pdf_path):
        ...
        try:
            ...
            # 7. 异常捕获，打印详细错误信息
            logger.error(f"===== 全流程测试运行失败 =====", exc_info=True)  # 等于logger.exception
